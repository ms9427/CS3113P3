...
const char SPRITESHEET_FILEPATH[] = "bee.png";
const char PLATFORM_1_FILEPATH[] = "flower.png";
const char PLATFORM_2_FILEPATH[] = "flytrap.png";
...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("Lunar Lander: Bee Addition",
    ...
    {
        //Setting Texture as Flower then Flytrap
        GLuint texture_id = (i % 2 == 0) ? platform_texture_1_id : platform_texture_2_id;
        g_state.platforms[i].m_texture_id = texture_id;
        ...
        //Setting Entity Type to either FLOWER or Flytrap 
        EntityType type = (i % 2 == 0) ? FLOWER : FLYTRAP;
        g_state.platforms[i].set_entity_type(type);
...
void process_input()
{
    ...
    if (key_state[SDL_SCANCODE_LEFT])
    {
        ...
        g_state.player->set_acceleration(glm::vec3(-drift, 0.0f, 0.0f));
        is_drifitng = true;
    }
    ...
    else if (key_state[SDL_SCANCODE_UP])
    {
        ...
        g_state.player->set_acceleration(glm::vec3(0.0f, 1.5f, 0.0f));
    }
    else
    {
        g_state.player->set_acceleration(glm::vec3(0.0f, gravity, 0.0f));
    ...
    if (!is_drifitng)
    {
        ...
        if (current_velocity.x > 0) {
            // Checking if velocity is positive meaning moving to right, then going add deceleration in opposite direction
            g_state.player->set_acceleration(glm::vec3(-deceleration, current_acceleration.y, 0.0f));
        }
        else if (current_velocity.x < 0) {
            // Checking if velocity is positive meaning moving to left, then going add deceleration in opposite direction
            g_state.player->set_acceleration(glm::vec3(deceleration, current_acceleration.y, 0.0f));
        }
    }
...
void render()
{
    ...
    else if (g_state.player->m_is_flying == false)
    {
        DrawText(&g_program, load_texture("font1.png"), "Mission Failed! ", 0.5f, -0.25f, glm::vec3(-1.2f, 2.75f, 0.0f));
        g_state.player->m_is_active = false;
    }
